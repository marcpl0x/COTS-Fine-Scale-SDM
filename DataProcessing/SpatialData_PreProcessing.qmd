...
library(gdistance)
library(readxl)
library(here)    # find files needed
```

## Read Data

```{r}
coralSDM.Ahya <- terra::rast("C:/Users/user/OneDrive - James Cook University/CCIP-BR-13-MarcKoh/CCIP-BR-13-MarcKoh/MarcData/maxent_predrast_GBR_AhyaD_015_lq2.tif")
coralSDM.Aspat <- terra::rast("C:/Users/user/OneDrive - James Cook University/CCIP-BR-13-MarcKoh/CCIP-BR-13-MarcKoh/MarcData/maxent_predrast_GBR_Aspat_015_lq2.tif")
coralSDM.Aten <- terra::rast("C:/Users/user/OneDrive - James Cook University/CCIP-BR-13-MarcKoh/CCIP-BR-13-MarcKoh/MarcData/maxent_predrast_GBR_Aten_015_lq2.tif")
crs(coralSDM.Ahya)
#John Brewer
...
env_files <- list.files(
  "C:/Users/user/OneDrive - James Cook University/CCIP-BR-13-MarcKoh/CCIP-BR-13-MarcKoh/MarcData/GBR_bathymetryDEM_ACA_UTM55_030_crop",
  pattern = "\\.tif$",
  full.names = TRUE
)
...
# Create res 50 dataset for env30
```

```{r add Turbidity Data from Allen Coral Atlas}

# all data from this section turb_[i], was extracted from Allen Coral Atlas custom polygon incorporating Torres Straits as well. 

# NOTE: THESE ARE YEARLY AVERAGES

library(tictoc)

# import data
#turb_19 <- terra::rast("C:/Users/user/OneDrive - James Cook #University/CCIP-BR-13-MarcKoh/CCIP-BR-13-MarcKoh/MarcData/GBR_TorStr_Turbidity/Turbidi#ty-2019/turbidity-annual_0.tif")
#turb_20 <- terra::rast("C:/Users/user/OneDrive - James Cook #University/CCIP-BR-13-MarcKoh/CCIP-BR-13-MarcKoh/MarcData/GBR_TorStr_Turbidity/Turbidity-#2020/turbidity-annual_0.tif")
#turb_21 <- terra::rast("C:/Users/user/OneDrive - James Cook #University/CCIP-BR-13-MarcKoh/CCIP-BR-13-MarcKoh/MarcData/GBR_TorStr_Turbidity/Turbidity-#2021/turbidity-annual_0.tif")
#turb_22 <- terra::rast("C:/Users/user/OneDrive - James Cook #University/CCIP-BR-13-MarcKoh/CCIP-BR-13-MarcKoh/MarcData/GBR_TorStr_Turbidity/Turbidity-#2022/turbidity-annual_0.tif")
#turb_23 <- terra::rast("C:/Users/user/OneDrive - James Cook #University/CCIP-BR-13-MarcKoh/CCIP-BR-13-MarcKoh/MarcData/GBR_TorStr_Turbidity/Turbidity-#2023/turbidity-annual_0.tif")
#turb_24 <- terra::rast("C:/Users/user/OneDrive - James Cook #University/CCIP-BR-13-MarcKoh/CCIP-BR-13-MarcKoh/MarcData/GBR_TorStr_Turbidity/Turbidity-#2024/turbidity-annual_0.tif")
#
## change names of raster layers
#names(turb_19) <- c("Turbidity 2019")
#names(turb_20) <- c("Turbidity 2020")
#names(turb_21) <- c("Turbidity 2021")
#names(turb_22) <- c("Turbidity 2022")
#names(turb_23) <- c("Turbidity 2023")
#names(turb_24) <- c("Turbidity 2024")

# update CRS to match the SDM ones

#tic()
#for (i in 19:24) {
#  var_name <- paste0("turb_", i)
#  layer_name <- get(var_name)
#  file_name <- paste0(var_name, ".tif")
#  
#  if (crs(layer_name) != crs(coralSDM.Ahya)) {
#    
#    message(paste("Reprojecting", var_name, "to match SDM CRS..."))
#    
#    z <- project(layer_name, crs(coralSDM.Ahya))
#    assign(var_name, layer_name)
#    
#    writeRaster(
#      z,
#      filename = file_name,
...
#  }
#  else{
#    message(paste(var_name, "has already been reprojected"))
#  }
#}
#toc() #90142.61 sec elapsed

# Reload the turbidiuty .tif files after reprojecting

turb_19 <- terra::rast("C:/Users/user/OneDrive - James Cook University/CCIP-BR-13-MarcKoh/COTS-Fine-Scale-SDM/DataProcessing/turb_19.tif")

turb_20 <- terra::rast("C:/Users/user/OneDrive - James Cook University/CCIP-BR-13-MarcKoh/COTS-Fine-Scale-SDM/DataProcessing/turb_20.tif")

turb_21 <- terra::rast("C:/Users/user/OneDrive - James Cook University/CCIP-BR-13-MarcKoh/COTS-Fine-Scale-SDM/DataProcessing/turb_21.tif")

turb_22 <- terra::rast("C:/Users/user/OneDrive - James Cook University/CCIP-BR-13-MarcKoh/COTS-Fine-Scale-SDM/DataProcessing/turb_22.tif")

turb_23 <- terra::rast("C:/Users/user/OneDrive - James Cook University/CCIP-BR-13-MarcKoh/COTS-Fine-Scale-SDM/DataProcessing/turb_23.tif")

turb_24 <- terra::rast("C:/Users/user/OneDrive - James Cook University/CCIP-BR-13-MarcKoh/COTS-Fine-Scale-SDM/DataProcessing/turb_24.tif")


# combine turbidiity data
turb_rast <- c(turb_19,turb_20,turb_21, turb_22, turb_23, turb_24)
#plot(turb_rast)

# Recrop for Sam's report:

turb_crop <- ext(145.8, 148.8, -19.4, -18.1)
turb_crop_poly <- as.polygons(turb_crop, crs = "EPSG:4326")
turb_crop_proj <- project(turb_crop_poly, crs(coralSDM.Ahya))

# crop turb_rast
turb_rast_crop <- terra::crop(turb_rast, turb_crop_proj)

# Quick resample
template30 <- env30[[1]]
turb_rast_crop <- resample(turb_rast_crop, template30, method = "bilinear") # template30 in line 221
turb_rast_crop <- crop(turb_rast_crop, turb_crop_proj)
res(turb_rast_crop) # check res is 30 x 30
beepr::beep()

# Write raster just in case
#writeRaster(
#  turb_rast_crop,
#  filename = "turb_rast_crop.tif",
#  overwrite = TRUE,
#  gdal = c("COMPRESS=LZW")  # optional compression
#)

# re-read in raster:
turb_rast_crop <- terra::rast("C:/Users/user/OneDrive - James Cook University/CCIP-BR-13-MarcKoh/COTS-Fine-Scale-SDM/DataProcessing/turb_rast_crop.tif")

# leaflet palette for turbidity
pal <- colorNumeric(
  palette = "viridis",
  domain = turb_rast_crop$`Turbidity 2019`
) # not working

# create leaflet map
turb_crop_map <- leaflet() %>% 
  addProviderTiles(providers$Esri.OceanBasemap, group = "Esri Ocean") %>% 
  addProviderTiles(providers$Esri.WorldImagery, group = "Esri Satellite") %>% 
  addProviderTiles(providers$CartoDB.Positron,  group = "Light") %>% 
  addRasterImage(turb_rast_crop[[1]], group = "2019", maxBytes = Inf) %>% 
  addRasterImage(turb_rast_crop[[2]], group = "2020", maxBytes = Inf) %>% 
  addRasterImage(turb_rast_crop[[3]], group = "2021", maxBytes = Inf) %>% 
  addRasterImage(turb_rast_crop[[4]], group = "2022", maxBytes = Inf) %>% 
  addRasterImage(turb_rast_crop[[5]], group = "2023", maxBytes = Inf) %>% 
  addRasterImage(turb_rast_crop[[6]], group = "2024", maxBytes = Inf) %>% 
  addLayersControl(baseGroups= c("Esri Ocean", "Esri Satellite", "Light"),
                  overlayGroups = c("2019", "2020","2021", "2022","2023", "2024"),
...
aus <- vect(st_transform(ozmap_data("country"), crs = crs(coralSDM.Ahya)))

aus_tsv <- crop(aus, turb_rast_crop)

tsv_label <- data.frame(
  lon = 146.816956,
  lat = -19.258965,
  label = "Townsville"
)
...
turb_crop_gg <- ggplot()+
  geom_spatvector(data = aus_tsv)+
  geom_spatraster(data = turb_rast_crop)+
  geom_point(data = tsv_label,
             aes(x = lon, y = lat))+
  geom_label(data = tsv_label,
             aes(x = lon, y = lat, label = label),
             vjust = 1.1,
             hjust = 1.1)+
  scale_fill_viridis_c( "Turbidity (FNU)" ,na.value = "transparent")+
...
#Align turbidity data:
turb_30 <- resample(turb,template30, method = "bilinear")

res(turb_30); res(template30) # check turbidity
all.equal(crs(turb_30),  crs(template30))
...
predictors_raster <- raster::stack(c(env30, coral_sdm_30,turb_30))
plot(predictors_raster[[13]])
```

```{r Create John Brewer Template Grid, include=FALSE}
template30 <- env30[[1]]
res(template30)   # should be ~30 m
ext(template30)
crs(template30)   # should now be EPSG:7855
...
# Align resolution/extent to the 30 m template
Ahya_30_crop  <- resample(coralSDM.Ahya_crop,  template30, method = "bilinear")
Aspat_30_crop <- resample(coralSDM.Aspat_crop, template30, method = "bilinear")
Aten_30_crop  <- resample(coralSDM.Aten_crop,  template30, method = "bilinear")

#Align turbidity data:
jb_turb_30 <- resample(jb_turb,template30, method = "bilinear") %>% 
  crop(.,bb_mga_poly)


all.equal(crs(Ahya_30_crop),  crs(template30))
all.equal(crs(Ahya_30_crop), crs(template30))
all.equal(crs(Ahya_30_crop),  crs(template30))
res(Ahya_30_crop); res(template30)

res(jb_turb_30); res(template30) # check turbidity
all.equal(crs(jb_turb_30),  crs(template30))

# keep env layers separate if you like
names(env30)  # rename if needed to sensible names

# recrop env30 for JB:
env30_jb <- crop(env30,bb_mga_poly)

coral_sdm_30_crop <- c(Ahya_30_crop, Aspat_30_crop, Aten_30_crop)
names(coral_sdm_30_crop) <- c("SDM_Ahya_crop", "SDM_Aspat_crop", "SDM_Aten_Crop")
coral_sdm_30_crop <- crop(coral_sdm_30_crop, bb_mga_poly)

predictors_terra_crop <- c(env30_jb, coral_sdm_30_crop)
predictors_terra_crop

predictors_raster_crop <- raster::stack(c(env30_jb, coral_sdm_30_crop, jb_turb_30))
plot(predictors_raster_crop)
...
reef_poly <- terra::vect("C:/Users/user/OneDrive - James Cook University/CCIP-BR-13-MarcKoh/CCIP-BR-13-MarcKoh/MarcData/Great_Barrier_Reef_Features.gpkg") |>
  filter(FEAT_NAME=="Reef") # M:changed filepath to suit me 
...
#make res coarser to 50m
res(predictors_reef) <- 50

#Geomorphic and Benthic Data
geomorph_rast <- rast("C:/Users/user/OneDrive - James Cook University/CCIP-BR-13-MarcKoh/CCIP-BR-13-MarcKoh/MarcData/GBR10_GBRMP_Geomorphic/GBR10 GBRMP Geomorphic.tif")
benthic_rast <- rast("C:/Users/user/OneDrive - James Cook University/CCIP-BR-13-MarcKoh/CCIP-BR-13-MarcKoh/MarcData/GBR10_GBRMP_Benthic/GBR10 GBRMP Benthic.tif")
...
writeRaster(
  predictors_reef,
  filename = "predictors_reef.tif",
  overwrite = TRUE,
  gdal = c("COMPRESS=LZW")  # optional compression
)
```
Is this the owen dataset??

```{r Load survey data}
#here::i_am("predictors_reef.tif")
#here()

# Quick load of predictors_reef:
predictors_reef <- terra::rast("predictors_reef.tif")
names(predictors_reef) <- c("benthic_class", "geomorphic_class")

plot(predictors_reef)

dat.cull <- read_excel(
  #"250929_COTS-Manta-Cull-RHIS-Data-Matthews-and-Schlawinsky.xlsx", sheet = 4)
  "C:/Users/user/OneDrive - James Cook University/CCIP-BR-13-MarcKoh/CCIP-BR-13-MarcKoh/MarcData/COTS-Manta-Cull-Data-Stewart.xlsx", sheet = 4)|>
...
MARC: the above chunk essentially means that we need to interpolate the data there. perhaps an `if()` function? If `NA` is shown in `resample()`, use `update(method = "ngb)`? Think about how to write this for loop.

Other option is to add 10 to the sample range each time it misses?

Also, John Brewer is not in the missing samples...Pick another reef?

**From Sam: plot using `leaflet()` to check how far apart the NAs are**

record NAs, have a look at distribution using histogram and summary stats
`gdist()`

<IGNORE CHUNK BELOW>
```{r reef predictors filtered, include = F}

#JB predictors 
predictors_raster

predictors_reef <- terra::mask(predictors_terra_crop, reef_poly)

plot(predictors_reef_jb)

benthic_rast_jb <- project(benthic_rast, predictors_reef_jb[[1]], method = "near")
geomorph_rast_jb <- project(geomorph_rast, predictors_reef_jb[[1]], method = "near")
benthic_rast_jb <- resample(benthic_rast, predictors_reef_jb[[1]], method = "near")
geomorph_rast_jb <- resample(geomorph_rast, predictors_reef_jb[[1]], method = "near")
names(benthic_rast_jb) <- "benthic_class"
names(geomorph_rast_jb) <- "geomorph_class"
# add to predictor stack
predictors_reef_jb <- c(predictors_reef_jb, benthic_rast_jb, geomorph_rast_jb)
names(predictors_reef_jb)

plot(benthic_rast_jb)
plot(geomorph_rast_jb)


# Try to resample at 50m:
res(predictors_reef_jb) <- 50

benthic_rast_jb <- terra::resample(benthic_rast, predictors_reef_jb[[1]], method = "near")
geomorph_rast_jb <- terra::resample(geomorph_rast, predictors_reef_jb[[1]], method = "near")

plot(geomorph_rast_jb)

dat.cull_jb <- dat.cull %>% filter(ReefName %in% "John Brewer Reef (18-075)")

dat.cull_jb_sf <- dat.cull_jb %>%  
  st_as_sf(
    coords = c("Longitude", "Latitude"),  # <- CHANGE if your cols are named differently
    crs    = 4326              # WGS84 geographic
  )
...
# so we transform the survey points to that CRS:
survey.jb_sf <- dat.cull_jb_sf |> st_transform(7844)

plot(survey.jb_sf)

survey.jb_sf <- survey.jb_sf %>%
  mutate(
...
survey.jb_sf <- st_transform(survey.jb_sf, crs(geomorph_rast_jb))

# 3.3 Extract raster values
pred_vals <- terra::extract(predictors_reef_jb, vect(survey.jb_sf))

# Try to resample at 100m:
res(predictors_reef_jb) <- 100
...
map1 <- leaflet() %>% 
  ...
  addProviderTiles(providers$Esri.WorldImagery, group = "Esri Satellite") %>% 
  addProviderTiles(providers$CartoDB.Positron,  group = "Light") %>% 
  addRasterImage(benthic_rast_jb, group = "Benth") %>% 
  addRasterImage(geomorph_rast_jb, group = "Geo") %>% 
  addLayersControl( baseGroups= c("Esri Ocean", "Esri Satellite", "Light"),
                    overlayGroups = c("Benth", "Geo","sample sites"),
                    options = layersControlOptions(collapsed = FALSE))%>% 
  leafem::addMouseCoordinates() %>% 
  addScaleBar(position = "bottomright")
...
print(is.na(geomorph_rast_jb$geomorph_class))
```
...
# Marc's packages
library(tictoc)       # timing runs
library(future)       # pralllel processing to save time (thanks Owen)
...
colnames(pred_vals)[2:3] <- c("benthic_class", "geomorphic_class")


# Infill the raster using spatial interpolation. NAs in benthic_class and geomorph_class
# count NAs
...
pred_vals$dist_to_benth <- 0
pred_vals$dist_to_geo   <- 0
# Initialize infill columns as NA (they will hold factor levels later)
pred_vals$infill_benth  <- NA
pred_vals$infill_geo    <- NA

pnts_vect <- vect(survey_sf) #vectorise survey_sf points

# Infill the geo layer (this first because it has more NAs)

na_geo <- which(is.na(pred_vals$geomorphic_class))

#Note: as.points() is pretty memory heavy

#future::availableCores() # 16 on Marc's computer
#future::plan(multisession)

tic()

if(length(na_geo) > 0) {
  r_pts_geo <- as.points(predictors_reef[["geomorphic_class"]], values=TRUE, na.rm=TRUE) # convert geo to points
  near_geo  <- nearest(pnts_vect[na_geo, ], r_pts_geo) # find nearest geo class
  
  pnts_na_geo <- pnts_vect[na_geo, ] # subset
    
  target_pts_geo <- r_pts_geo[near_geo$to_id, ]
  distances <- distance(pnts_na_geo, target_pts_geo, pairwise = TRUE) # Calculate distances for the NA points
  within_limit_idx <- which(distances <= 100) # set limit of 100 m
    
  if (length(within_limit_idx) > 0) {
    replacement_labels <- values(target_pts_geo[within_limit_idx, ])[,1] # retrieve geo class if within 100 m
    original_idx <- na_geo[within_limit_idx]
    pred_vals$geomorphic_class[original_idx] <- replacement_labels
    pred_vals$infill_geo[original_idx] <- as.character(replacement_labels)
    }
  
  pred_vals$dist_to_geo[na_geo] <- distances # retrieve all distances just in case
  }
#  },

# potentially clear memory here
gc()

# Now do benthic layers, see geo layer for comments on each part
na_benth <- which(is.na(pred_vals$benthic_class))

tic()
if(length(na_benth) > 0) {
  r_pts_benth <- as.points(predictors_reef[["benthic_class"]], values=TRUE, na.rm=TRUE)
  near_benth  <- nearest(pnts_vect[na_benth, ], r_pts_benth)
  
  pnts_na_benth <- pnts_vect[na_benth, ]
  
  target_pts_benth <- r_pts_benth[near_benth$to_id, ]
  distances_benth <- distance(pnts_na_benth, target_pts_benth, pairwise = TRUE)
  within_limit_idx_benth <- which(distances_benth <= 100)
  
  if (length(within_limit_idx_benth) > 0) {
    replacement_labels_benth <- values(target_pts_benth[within_limit_idx_benth, ])[,1]
    original_idx_benth <- na_benth[within_limit_idx_benth]
    pred_vals$benthic_class[original_idx_benth] <- replacement_labels_benth
    pred_vals$infill_benth[original_idx_benth]  <- as.character(replacement_labels_benth)
  }
  
  # Assign ALL calculated distances
  pred_vals$dist_to_benth[na_benth] <- distances_benth
}
toc()

# save it just incase
#write.csv(pred_vals, file = "pred_vals.csv")
...
survey_df <- survey_sf %>%
  ...
  tidyterra::left_join(.,pred_vals, by = "ID") %>% 
  dplyr::select(!`X`)
...
# Quick check
glimpse(survey_df)
#write.csv(survey_df, file = "survey_df.csv")
#survey_df <- read.csv("C:/Users/user/OneDrive - James Cook University/CCIP-BR-13-MarcKoh/COTS-Fine-Scale-SDM/DataProcessing/survey_df.csv")


# How many NAs now?
sum(is.na(survey_df$benthic_class))
sum(is.na(survey_df$geomorphic_class))
...
set.seed(123)
sb <- spatialBlock( # This function is deprecated! Please use 'cv_spatial' instead.
...
Marc: Next chunk plots the same tsv turbidity coordinates as per Sam's request on teams on 9/2/2026
```{r tsv benth and geo classes}
# This assumes that line 732 has been run
sf_bg <- survey_sf %>% 
...
plot(v_bg,"benthic_class")
plot(v_bg,"geomorphic_class")

crs(sf_bg)
plot(sf_bg["benthic_class"])
plot(sf_bg["geomorphic_class"])

v_bg_tsv <- crop(v_bg, turb_rast_crop)
...
library(MetBrewer)
tsv_benth <- ggplot()+
  geom_spatvector(data = v_bg_tsv, 
                  aes(colour = v_bg_tsv$benthic_class))+
  geom_spatvector(data = aus_tsv)+
  scale_colour_manual("Benthic Class", values = met.brewer("Kandinsky"))+
  #geom_point(data = tsv_label, 
  #           inherit.aes = F,
  #           aes(lon, lat))+
  theme_classic()
...
tsv_geo <- ggplot()+
  geom_spatvector(data = v_bg_tsv, 
                  aes(colour = v_bg_tsv$geomorphic_class))+
  geom_spatvector(data = aus_tsv)+
  scale_colour_viridis_d("Geomorphic Class")+
  #geom_point(data = tsv_label, 
  #           inherit.aes = F,
  #           aes(lon, lat))+
  theme_classic()
